RelativeFormatter = class RelativeFormatter{
    ...
    static getKeyAndCount(date,precision = RelativeFormatter.Precision.Second,idiomatic = false){
        var now = new Date();
        var dateComponents = RelativeFormatter.componentsDifference(date,now);
        var timeKey = "ahead";
        ...
        else if(dateComponents.minutes == 0 && precision == RelativeFormatter.Precision.Minute){
            return {key:"thisminute",count:0};
        }else if(dateComponents.seconds > 0){
            if(dateComponents.seconds == 1 && idiomatic){
